...
struct AppStoreConnectCommand: AsyncParsableCommand {
    ...
    func run() async throws {
        ...
        guard let keyID = ProcessInfo.processInfo.environment["ASC_KEY_ID"],
              ...
              let privateKeyPath = ProcessInfo.processInfo.environment["ASC_PRIVATE_KEY_PATH"] else {
            logger.error("Missing required environment variables: ASC_KEY_ID, ASC_ISSUER_ID, ASC_PRIVATE_KEY_PATH")
            throw ExitCode.failure
        }
        
        let keyExpiry = ProcessInfo.processInfo.environment["ASC_KEY_EXPIRY"]
